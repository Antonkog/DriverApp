 ...
 */
class ApiServiceWrapper(private val api: ApiService, private val localDataSource: LocalDataSource, private val context: Context , val gson : Gson) {
    ...
    suspend fun updateTasksFromServer(deviceId: String){
        ...
        val change = ChangeHistory(Status.SENT, LogType.APP_TO_SERVER, HistoryDataType.GET_TASKS, deviceId, null, time,time)
        RxBus.publish(RxBusEvent.RequestStatus(MainViewModel.Status(null, MainViewModel.StatusType.LOADING)))
        ...
        if (remoteTasks.isSuccess && !remoteTasks.isException) {
            ...
            localDataSource.updateHistoryChange(change.copy(status = Status.SUCCESS, response = gson.toJson(remoteTasks)))
            ...
            localDataSource.updateHistoryChange(change.copy(status = Status.ERROR))
    ...
    suspend fun setDeviceProfile(commItem: CommItem?): ResultWrapper<ResultOfAction>{
        RxBus.publish(RxBusEvent.RequestStatus(MainViewModel.Status(null, MainViewModel.StatusType.LOADING)))
        val time =  System.currentTimeMillis()
        val change = ChangeHistory(Status.SENT, LogType.APP_TO_SERVER, HistoryDataType.SET_DEVICE_PROFILE, gson.toJson(commItem),null,  time, time)
        val autoGenId =  localDataSource.insertHistoryChange(change)
        return try {
            val result =
                ResultWrapper.Success(api.setDeviceProfile(commItem))
            RxBus.publish(RxBusEvent.RequestStatus(MainViewModel.Status(result.toString(), MainViewModel.StatusType.COMPLETE)))
            localDataSource.updateHistoryChange(change.copy(status = Status.SUCCESS, response = gson.toJson(result), id = autoGenId))
            result
        } catch (ex: Exception){
            RxBus.publish(RxBusEvent.RequestStatus(MainViewModel.Status(ex.message, MainViewModel.StatusType.ERROR)))
    ...
    suspend fun postActivityChange(commItem: CommItem): ResultWrapper<ResultOfAction> {
        RxBus.publish(RxBusEvent.RequestStatus(MainViewModel.Status(null, MainViewModel.StatusType.LOADING)))
        val time =  System.currentTimeMillis()
        val change = ChangeHistory(Status.SENT, LogType.APP_TO_SERVER, HistoryDataType.POST_ACTIVITY, gson.toJson(commItem),null,  time, time)
        val autoGenId = localDataSource.insertHistoryChange(change)
        return try {
    ...
    suspend fun confirmTask(commItem: CommItem): ResultWrapper<ResultOfAction>{
        val time =  System.currentTimeMillis()
        val change = ChangeHistory(Status.SENT, LogType.APP_TO_SERVER, HistoryDataType.CONFIRM_TASK, gson.toJson(commItem),null,  time, time)
        val autoGenId =  localDataSource.insertHistoryChange(change)
        return try{
