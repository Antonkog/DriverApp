...
/**
 * A singleton wrapper around {@link ThreadPoolExecutor} that helps to post
 * work on a background worker thread.
 */
public class ThreadExecutor {
  
  private static ThreadExecutor sINSTANCE = null;
  ...
  private ThreadExecutor() {
    // initialize a queue for the thread pool. New tasks will be added
    // to this queue.
    mTaskQueue = new LinkedBlockingQueue<Runnable>();
    mRunningTaskList = new ArrayList<>();
    
    Log.e("ThreadExecutor", "Available cores: " + NUMBER_OF_CORES);
  ...
  /**
   * Add a callable to the queue, which will be executed by the next available
   * thread in the pool.
   */
  ...
  /**
   * A ThreadFactory implementation which create new threads for the thread
   * pool. The threads created is set to background priority, so it does not
   * compete with the UI thread.
   */
  private static class BackgroundThreadFactory implements ThreadFactory {
    ...
    public Thread newThread(Runnable runnable) {
      ...
      thread.setName("DriverThread" + sTag);
      ...
      thread.setUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
        ...
        public void uncaughtException(Thread thread, Throwable ex) {
          Log.e("ThreadExecutor", thread.getName() + " encountered an error: " + ex.getMessage());
        }
      });
      return thread;
    }
