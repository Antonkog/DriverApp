...
public final class ThreadUtils {
    ...
    synchronized (sLock) {
      ...
      if (sUiThreadHandler != null && sUiThreadHandler.getLooper() != looper) {
        throw new RuntimeException("UI thread looper is already set to "
          ...
          + Looper.getMainLooper() + "), cannot set to new looper "
          + looper);
      } else {
    ...
    if (runningOnUiThread()) {
      ...
      try {
        ...
        throw new RuntimeException(
          "Exception occurred while waiting for runnable", e);
      }
    }
  }
  
  /**
   * Run the supplied Callable on the main thread, wrapping any exceptions
   * in a RuntimeException. The method will block until the Callable
   * completes.
   *
   ...
   *
   * @param task The Runnable to run.
   * @param delayMillis The delay in milliseconds until the Runnable will
   * be run.
   */
  ...
  /**
   * Throw an exception if currently not running on the UI thread.
   */
  ...
  /**
   * Throw an exception (regardless of build) if currently not running
   * on the UI thread.
   */
   ...
   *
   * Can be used by tests where code that normally runs multi-threaded is
   * going to run single-threaded for the test (otherwise asserts that are
   * valid in production would fail in those tests).
   */
  ...
  public static boolean runningOnUiThread() {
    return (getUiThreadLooper() == Looper.myLooper());
  }
   ...
   */
  public static void
  setThreadPriorityAudio(int tid) {
